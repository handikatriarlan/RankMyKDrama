...
const DramaInput = forwardRef<HTMLInputElement, Props>(
  ({ onDramaSelect, disabled, selectedDramas }, ref) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowUp':
          e.preventDefault()
          setSelectedIndex(
            (prev) => (prev - 1 + filteredDramas.length) % filteredDramas.length
          )
          break
        case 'Enter':
          e.preventDefault()
      ...
      <div className='relative w-full max-w-2xl'>
        <div className='relative'>
          <input
            ...
            placeholder={
              disabled
                ? 'Maximum dramas reached!'
                : 'Type to search K-Drama titles...'
            }
            value={search}
            onChange={(e) => {
        ...
        {showDropdown && search && (
          <div
            ref={dropdownRef}
            className='absolute z-10 w-full mt-2 bg-white rounded-xl shadow-xl border border-purple-100 max-h-[300px] overflow-auto'
          >
            {filteredDramas.map((drama, index) => (
              <button
                key={drama.title}
                className={`w-full px-4 py-3 text-left hover:bg-purple-50 flex items-center gap-3 group transition-colors duration-200
              ...
              >
                <img
                  src={drama.image}
                  alt={drama.title}
                  className='w-12 h-12 rounded-lg object-cover group-hover:ring-2 ring-purple-400 transition-all duration-200'
                />
                <div>
                  <p className='font-medium text-gray-800'>{drama.title}</p>
